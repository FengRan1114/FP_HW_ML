...
# 加载特征和标签
features = np.load("fuxian_features.npy")
labels = np.load("fuxian_labels.npy")

# PCA 降维（可选）
# pca = PCA(n_components=512)  # 试试 512 维，或注释掉
...
classifiers = {
    ...
    # "XGB": xgb.XGBClassifier(max_depth=5, learning_rate=0.05, n_estimators=200, eval_metric="mlogloss")
}

# 5 折交叉验证
kf = KFold(n_splits=5, shuffle=True, random_state=42)
num_runs = 50
final_results = {name: [] for name in classifiers.keys()}
final_f1_scores = {name: [] for name in classifiers.keys()}
...
for run in range(num_runs):
    ...
    for name, clf in classifiers.items():
        ...
        final_results[name].append(mean_accuracy)
        final_f1_scores[name].append(mean_f1)

print("\n最终结果（50次运行的平均准确率和F1分数）：")
for name, accuracies in final_results.items():
    final_accuracy = np.mean(accuracies)
    final_f1 = np.mean(final_f1_scores[name])
    print(f"{name}: 平均准确率 {final_accuracy:.4f}, 平均 F1 {final_f1:.4f}")

# 保存混淆矩阵
for name, cms in all_confusion_matrices.items():
    avg_cm = np.mean(cms, axis=0)
    cm_df = pd.DataFrame(avg_cm, index=range(15), columns=range(15))
    cm_df.to_csv(f"E:\\python project\\Believe\\vgg16_confusion_matrix_{name}.csv")
...
for name, cms in all_confusion_matrices.items():
    ...
    plt.title(f"VGG16 {name} Confusion Matrix", fontsize=14, fontfamily="Arial")
    plt.xlabel("Predicted Label", fontsize=12, fontfamily="Arial")
    plt.ylabel("True Label", fontsize=12, fontfamily="Arial")
    plt.savefig(f"E:\\python project\\Believe\\vgg16_confusion_matrix_{name}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
for name, roc_data in all_roc_data.items():
    if roc_data:
        ...
        plt.title(f"VGG16 {name} ROC Curve", fontsize=14, fontfamily="Arial")
        plt.legend(loc="lower right", fontsize=10)
        plt.savefig(f"E:\\python project\\Believe\\vgg16_roc_curve_{name}.png", dpi=300, bbox_inches="tight")
        plt.close()