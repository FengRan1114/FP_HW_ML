...
class PUF_GNN(nn.Module):
    def __init__(self, in_dim=2048, hidden_dim=512):
        ...
        self.group_classifier = nn.Linear(hidden_dim, 3)
        self.nongroup_classifier = nn.Linear(hidden_dim, 1)
...
for run in range(num_runs):
    ...
    for fold in range(num_folds):
        # 加载特征
        train_features = np.load(f"E:\\python project\\Believe\\train_features_fold_{fold}_resnet.npy",
                                 allow_pickle=True).item()
        test_features = np.load(f"E:\\python project\\Believe\\test_features_fold_{fold}_resnet.npy",
                                allow_pickle=True).item()

        missing_devices = [d for d in devices if d not in train_features]
        if missing_devices:
            print(f"Error: Missing devices in Run {run + 1}, Fold {fold + 1}: {missing_devices}")
            continue
        ...
        # 创建数据
        train_data = Data(x=train_x, edge_index=dynamic_edge_index, y_group=group_labels, y_nongroup=nongroup_labels)
        test_data = Data(x=test_x, edge_index=dynamic_edge_index, y_group=group_labels, y_nongroup=nongroup_labels)

        # 初始化模型
        model = PUF_GNN(in_dim=2048)
        ...
        for epoch in range(200):
            optimizer.zero_grad()
            group_logits, nongroup_logits = model(train_data.x, train_data.edge_index)
        ...
        model.eval()
        with torch.no_grad():
            group_logits, nongroup_logits = model(test_data.x, test_data.edge_index)
            ...
            nongroup_f1 = f1_score(test_data.y_nongroup.numpy(), nongroup_pred.numpy())
...
acc_df.to_csv("E:\\python project\\Believe\\resnet_accuracies.csv")
...
group_pred_df.to_csv("E:\\python project\\Believe\\resnet_group_predictions.csv")
nongroup_pred_df = pd.DataFrame([
    {"Run": pred["Run"], "Fold": pred["Fold"], "Device": devices[i], "True": pred["True"][i], "Pred": pred["Pred"][i]}
    for pred in all_nongroup_predictions for i in range(len(pred["True"]))
])
nongroup_pred_df.to_csv("E:\\python project\\Believe\\resnet_nongroup_predictions.csv")

# 保存混淆矩阵
avg_group_cm = np.mean(all_group_cm, axis=0)
avg_nongroup_cm = np.mean(all_nongroup_cm, axis=0)
pd.DataFrame(avg_group_cm, index=["A", "B", "C"], columns=["A", "B", "C"]).to_csv("E:\\python project\\Believe\\resnet_group_cm.csv")
pd.DataFrame(avg_nongroup_cm, index=["Normal", "Malicious"], columns=["Normal", "Malicious"]).to_csv("E:\\python project\\Believe\\resnet_nongroup_cm.csv")

print("\n最终结果（ResNet50, 50 runs）：")
...
plt.title("ResNet50 Group Confusion Matrix", fontsize=14, fontfamily="Arial")
plt.xlabel("Predicted Group", fontsize=12, fontfamily="Arial")
plt.ylabel("True Group", fontsize=12, fontfamily="Arial")
plt.savefig("E:\\python project\\Believe\\resnet_group_cm.png", dpi=300, bbox_inches="tight")
plt.close()
...
plt.title(f"ResNet50 Malicious Node ROC (AUC = {np.mean([r[2] for r in all_nongroup_roc]):.2f})", fontsize=14, fontfamily="Arial")
plt.savefig("E:\\python project\\Believe\\resnet_nongroup_roc.png", dpi=300, bbox_inches="tight")
plt.close()
...
plt.title("ResNet50 Group and Malicious Node Network", fontsize=14, fontfamily="Arial")
plt.savefig("E:\\python project\\Believe\\resnet_group_network.png", dpi=300, bbox_inches="tight")
plt.close()